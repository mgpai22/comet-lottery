...
class serviceOwnerConf(
  ...
  private val conf = ServiceOwnerConfig(
    ownerMnemonic,
    ownerMnemonicPw,
    cometId,
    cometTicketPrice,
...
class conf(
  ...
  val ticketContractInstance: TicketContract =
    TicketContract(ticketContract, singleton, initialTCBox)
  val collectionContractInstance: CollectionContract =
    CollectionContract(collectionContract, initialCCBox)
  val proxyContractInstance: ProxyContract = ProxyContract(proxyContract)
  val winnerSelectionContractInstance: WinnerSelectionContract =
    WinnerSelectionContract(winnerSelectionContract)
  val conf = Config(
    version,
    "null",
    ...
    "true",
    timeStamp,
    ticketContract = ticketContractInstance,
    collectionContract = collectionContractInstance,
    proxyContract = proxyContractInstance,
    winnerSelectionContract = winnerSelectionContractInstance,
    winningNFT = "null",
    winningAddress = "null"
  )
...
object conf {
  ...
  def writeTx(filePath: String, initTx: String): Unit = {
    ...
    val conf = Config(
      res.Lottery.version,
      initTx,
      res.Lottery.distributionAddress,
      res.Lottery.firstTime,
      res.Lottery.timeStamp,
      ticketContract = res.Lottery.ticketContract,
      collectionContract = res.Lottery.collectionContract,
      proxyContract = res.Lottery.proxyContract,
      winnerSelectionContract = res.Lottery.winnerSelectionContract,
      winningNFT = res.Lottery.winningNFT,
      winningAddress = res.Lottery.winningAddress
    )
    val newConfig: LotteryConfig = LotteryConfig(conf)
